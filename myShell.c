...
void addToken(cmd *c, char *inpToken)
{
    ...
    // for (int i = 0; inpToken[i] != 0; i++){

    //     c->token[i].ch[i] = inpToken[i];
    // }
    ...
    int wild_strlen = strlen (wild_string);
        ...
        while((search_item = readdir(search_directory)) != NULL){
            ...
            if (wild_equals(wild_part, real_name)){
                ...
                memset(&new->token[wild_ind].ch, 0, 256);
...
int execute(cmd *cmd, int in, int out)
{
    ...
    {
        char buff[257];
        getcwd(buff, 256); // verify pwd works
        printf("%s\n", buff);
        return EXIT_SUCCESS;
    }
...
void prtCmd(cmd *prt)
{
    // if(prt->tokens==8)prt->tokens++; // DELETE THIS LINE

    char buff [500];
    getcwd(buff, 499);
    printf("%s\n ", buff);
    for (int i = 0; i < prt->tokens; i++)
    {
        printf("%s ", prt->token[i].ch);
    }
    printf("\n");
...
int main(int argc, char *argv[])
{
    ...
    cmd *currCmd = newCmd();
    char currTok[256]; // (TO_DO) make token size expandable
    memset(currTok, 0, 256);
    char ch;
    if (interactive)
    {
